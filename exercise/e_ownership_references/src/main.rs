...
// "accepts ownership" means that the value is being moved from one variable
// to the one defined in the function.
// Since this function isn't taking a reference type, the value being passed in
// when the function is called is moved from to s in this function
fn eat(s: String) -> bool {
    if s.starts_with("b") && s.contains("a") {
        true // <-- tail expression
    } else {
        false // <-- tail expression
    }
}
